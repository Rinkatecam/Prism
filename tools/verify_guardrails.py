...
suite(
    ...
    Mutation("reduced motion is read and then ignored",
             ...
             "    return beating() && unacknowledged && !document.hidden;",
    ...
    Mutation("the loop can re-arm while the tab is hidden",
             "static/js/vitals-monitor.js",
             "    return !reduceMotion && beating() && unacknowledged && !document.hidden;",
             "    return !reduceMotion && beating() && unacknowledged;",
...
suite(
    ...
    Mutation("the squeeze runs off a second clock",
             ...
             "      heartEl.dataset.beatPhase = String(beatU);",
             "test_the_squeeze_is_driven_from_the_trace_s_own_frame"),
    Mutation("a still heart freezes mid-contraction",
             "static/js/vitals-monitor.js",
             "still ? '0' : squeeze(beatU).toFixed(3)",
             "squeeze(beatU).toFixed(3)",
    ...
    Mutation("an acknowledged change keeps beating",
             "static/js/vitals-monitor.js",
             "    return !reduceMotion && beating() && unacknowledged && !document.hidden;",
             "    return !reduceMotion && beating() && !document.hidden;",
             "test_an_acknowledged_state_does_not_beat"),
    ...
    Mutation("the acknowledgement key forgets which problem it is",
             ...
             "    return ACK_KEY + ':' + severity;",
             "test_acknowledgement_is_keyed_on_what_is_wrong_not_just_how_bad"),
    Mutation("a throwing store is read as acknowledged, silencing the beat",
             "static/js/vitals-monitor.js",
             "      // something changed is a missed outage. Over-signal, never under-signal.\n      return false;",
             "      // something changed is a missed outage. Over-signal, never under-signal.\n      return true;",
             "test_an_unavailable_store_fails_towards_beating"),
    ...
    Mutation("start() decides for itself instead of asking one predicate",
             "static/js/vitals-monitor.js",
             "    const run = animating();",
             "    const run = !reduceMotion && beating() && !document.hidden;",
             "test_the_decision_to_move_is_made_in_one_place"),
