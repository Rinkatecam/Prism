...
suite(
    ...
    # WP-4 D1b: the save posts only what the page owns.
    Mutation("the payload is seeded from the fetched config again",
             "templates/settings.html",
             "    const data = { settings: {} };",
             "    const data = current; data.settings = data.settings || {};",
             "test_the_payload_starts_empty_rather_than_from_the_fetched_config"),
    Mutation("a sub-tree this page does not render creeps into the payload",
             "templates/settings.html",
             "    // General\n    data.settings.poll_interval_seconds",
             "    data.settings.thresholds = current.settings?.thresholds || {};\n"
             "    // General\n    data.settings.poll_interval_seconds",
             "test_the_page_posts_only_the_settings_it_owns"),
    Mutation("the server list is echoed back and stops being preserved wholesale",
             "templates/settings.html",
             "    data.settings.https = {",
             "    data.servers = current.servers;\n    data.settings.https = {",
             "test_the_server_list_is_not_posted_back"),
    Mutation("auth stops being merged, blanking backup_admin and the lockout keys",
             "templates/settings.html",
             "data.settings.auth = Object.assign({}, current.settings?.auth || {}, {",
             "data.settings.auth = Object.assign({}, {",
