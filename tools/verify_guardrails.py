...
suite(
    ...
    Mutation("base.html stops owning a helper two pages call",
             "templates/base.html",
             "window.doTestConnection = function",
             "window.doTestConnectionX = function",
             "test_base_html_still_owns_the_helpers_pages_depend_on"),
    Mutation("a page takes its own copy of a shared helper again",
             "templates/servers.html",
             "function testConnection(index) {",
             "function loadConfig() { return fetch('/api/config').then(r => r.json()); }\n"
             "function testConnection(index) {",
             "test_no_page_redefines_a_helper_base_html_owns"),
    Mutation("the helper list is emptied so the guard passes vacuously",
             "tests/test_action_dispatch.py",
             '"_escHtml": "three copies existed until a move took one page\'s only one",',
             "",
             "test_the_helper_list_still_names_the_ones_that_matter"),
...
suite(
    ...
    Mutation("a popup goes back to a raw colour literal",
             "templates/partials/settings/_dependencies.html",
    ...
    # ── D4c: dependencies ──────────────────────────────────
    Mutation("a dependency function is left behind on /servers",
             "templates/servers.html",
             "// Dependencies moved to Settings",
             "function loadDepsTable() {}\n// Dependencies moved to Settings",
             "test_dependencies_left_the_servers_page"),
    Mutation("a dependency function is lost in the move",
             "templates/partials/settings/_dependencies.html",
             "function showDepBrowsePopup(",
             "function showDepBrowsePopupX(",
             "test_the_dependency_editor_is_whole_at_its_destination"),
    Mutation("a dependency shim is left behind, so its control goes dead",
             "templates/partials/settings/_dependencies.html",
             "function _srvFilterDepBrowse()",
             "function _srvFilterDepBrowseX()",
             "test_every_dependency_shim_travelled_with_its_function"),
    Mutation("nothing loads the dependency table on its own page",
             "templates/partials/settings/_dependencies.html",
             "if (document.getElementById('deps-table-container')) loadDepsTable();",
             "// it loads itself, surely",
             "test_the_destination_loads_its_own_table"),
    Mutation("the dependency bootstrap call comes back to /servers",
             "templates/servers.html",
             "  loadServerTags();",
             "  loadDepsTable();\n  loadServerTags();",
             "test_no_dependency_bootstrap_call_is_left_behind"),
    Mutation("the script-built dependency empty state is hand-rolled again",
             "templates/partials/settings/_dependencies.html",
             "container.innerHTML = window.prismEmptyState(",
             "container.innerHTML = '<p>None</p>'; const _x = (",
             "test_both_dependency_empty_states_use_the_shared_renderer"),
    Mutation("a dependency fallback drifts from its locale entry",
             "templates/partials/settings/_dependencies.html",
             "'Define dependencies in Servers settings to map service relationships')) }}",
             "'Define dependencies here')) }}",
             "test_the_dependency_hint_was_not_defined_a_second_time"),
    # ── D4d: add / edit / delete ──────────────────────────────
    Mutation("a configuration function is left behind on the fleet page",
             "templates/servers.html",
             "function showGuide() {",
             "function saveServer() {}\nfunction showGuide() {",
             "test_the_configuration_functions_left_the_fleet_page"),
    Mutation("the action button is dragged out with the configuration",
             "templates/servers.html",
             "function testConnection(index) {",
             "function testConnectionMoved(index) {",
             "test_the_actions_and_the_view_stayed"),
    Mutation("an edit button is left on the fleet page",
             "templates/servers.html",
             '<button data-action="testConnection"',
             '<button data-action="editServer" data-args="[0]"></button>\n'
             '              <button data-action="testConnection"',
             "test_no_configuration_control_is_left_on_the_fleet_page"),
    Mutation("the fleet page stops saying where its configuration went",
             "templates/servers.html",
             '<a href="/settings/servers"',
             '<a href="/servers"',
             "test_the_fleet_page_says_where_its_configuration_went"),
    Mutation("the settings table loses the rows that act",
             "templates/partials/settings/_server_config.html",
             'id="settings-server-table"',
             'id="settings-server-table-disabled"',
             "test_the_settings_page_has_a_table_to_act_on"),
    Mutation("the setup guide travels with the modals",
             "templates/partials/settings/_server_config.html",
             "function populateCustomServerTypes() {",
             'function closeGuide() {}\nfunction populateCustomServerTypes() {',
             "test_the_setup_guide_did_not_travel"),
    Mutation("a moved name is still evaluated on /servers as a callback",
             "templates/servers.html",
             "document.addEventListener('keydown', e => {",
             "document.addEventListener('DOMContentLoaded', populateCustomServerTypes);\n"
             "document.addEventListener('keydown', e => {",
             "test_no_moved_name_is_still_evaluated_on_the_fleet_page"),
    Mutation("the fleet page reaches for a modal it no longer has",
             "templates/servers.html",
             "  if (e.key === 'Escape') closeGuide();",
             "  if (e.key === 'Escape' && document.getElementById('server-modal')) closeGuide();",
             "test_escape_still_closes_something_on_each_page"),
    Mutation("the fleet empty state points at buttons that moved",
             "i18n.py",
             "'no_servers_hint_fleet': 'Add the first one under Settings",
             "'no_servers_hint_fleet': 'Click \"Add Server\" under Servers",
             "test_the_fleet_empty_state_names_the_new_place"),
...
# ── report exports: the file someone opens ──────────────────────
suite(
    "report-exports",
    Mutation("the BOM is dropped, so Excel reads UTF-8 as CP1252",
             "routes/api/reports.py",
             'body = "\\ufeff" + csv_text',
             'body = csv_text',
             "test_every_csv_starts_with_a_utf8_bom"),
    Mutation("the charset is spelled twice in the header again",
             "routes/api/reports.py",
             'mimetype="text/csv",',
             'mimetype="text/csv; charset=utf-8",',
             "test_every_csv_declares_utf8_exactly_once"),
    Mutation("timestamps go back to ISO-with-Z, which Excel reads as text",
             "routes/api/reports.py",
             'return dt.astimezone(zoneinfo.ZoneInfo(name)).strftime("%Y-%m-%d %H:%M:%S")',
             'return str(iso_str)',
             "test_timestamps_are_a_shape_excel_parses_as_a_date"),
    Mutation("the timestamp column stops naming its timezone",
             "reports.py",
             'writer.writerow(["Server", f"Timestamp ({ts_label})", "CPU %", "RAM %",',
             'writer.writerow(["Server", "Timestamp", "CPU %", "RAM %",',
             "test_the_timestamp_column_names_its_timezone"),
    Mutation("a cell Excel would evaluate is emitted unescaped",
             "reports.py",
             'row["server"], fmt(row["timestamp"]), row["type"], row["metric"],',
             'row["server"], fmt(row["timestamp"]), "=1+1", row["metric"],',
             "test_no_cell_would_be_evaluated_as_a_formula"),
    Mutation("a table goes back to overflowing the frame",
             "reports.py",
             "colWidths=[81, 100, 100, 100, 100])",
             "colWidths=[90, 105, 105, 105, 105])",
             "test_no_table_is_wider_than_the_page"),
    Mutation("stored text goes back into a fixed-width string cell",
             "reports.py",
             '_cell(e.get("message", ""), _msg_style(8)),',
             'e.get("message", "")[:80],',
             "test_no_stored_text_is_placed_in_a_plain_string_cell"),
    Mutation("the wrapping cell stops escaping its input",
             "reports.py",
             'return Paragraph(escape(str(text or ""))[:300], style)',
             'return Paragraph(str(text or "")[:300], style)',
             "test_a_message_containing_markup_does_not_break_the_document"),
    Mutation("a sparkline overhangs its own cell again",
             "reports.py",
             "_draw_sparkline(vals, width=88, height=25,",
             "_draw_sparkline(vals, width=100, height=25,",
             "test_every_sparkline_fits_inside_its_own_cell"),
    Mutation("the PDF stops being A4",
             "reports.py",
             "pagesize=A4",
             "pagesize=letter",
             "test_the_pdf_is_a4"),
...
# ── firewall events reaching the table ─────────────────────────
suite(
    "firewall-ingest",
    Mutation("the cap goes back to clipping the last channel",
             "ingest_caps.py",
             '"max_rows_per_check": 120',
             '"max_rows_per_check": 100',
             "test_the_cap_does_not_starve_the_last_channel"),
    Mutation("firewall policy events fall off the allowlist again",
             "config_manager.py",
             '"Firewall/2004",   # a rule was added',
             '# "Firewall/2004",',
             "test_firewall_policy_events_are_allowed_through_ingest"),
    Mutation("the per-packet flood events are allowlisted",
             "config_manager.py",
             '"Firewall/2033",   # all rules deleted',
             '"Firewall/2033", "Firewall/5152",',
             "test_the_per_packet_events_are_still_kept_out"),
    Mutation("the noise filter goes back to the channel without the noise",
             "collector_v2/scripts.py",
             "if ($displayName -eq 'Firewall' -or $displayName -eq 'Security') {",
             "if ($displayName -eq 'Firewall') {",
             "test_the_packet_noise_filter_covers_the_security_channel"),
    Mutation("an unreachable selection bucket comes back",
             "collector_v2/scripts.py",
             "$buckets = @{0=@(); 1=@(); 3=@()}",
             "$buckets = @{0=@(); 1=@(); 2=@(); 3=@()}",
             "test_no_selection_bucket_is_unreachable"),
)
...
suite(
    "evidence-report",
    Mutation("an unread Defender is reported as disabled again",
             "evidence.py",
             "if engine in (None, \"\", _ENGINE_UNREAD) and sig_age in (None, _SIG_AGE_UNREAD, -1):",
             "if False:",
             "test_an_unread_defender_is_reported_as_unknown_not_disabled"),
    Mutation("a genuinely disabled Defender stops being reported",
             "evidence.py",
             "            if not dfn:\n                concerns.append(\"Defender disabled\")",
             "            if False:\n                concerns.append(\"Defender disabled\")",
             "test_a_genuinely_disabled_defender_is_still_reported"),
    Mutation("the fleet-wide collection warning is dropped",
             "evidence.py",
             '"collection_warning": (\n            None if not unmeasured_all else',
             '"collection_warning": (\n            None if True else',
             "test_the_report_warns_when_a_field_was_unreadable_fleet_wide"),
    Mutation("a broken audit chain is reported as intact",
             "evidence.py",
             'elif chain.get("ok"):',
             'elif True:',
             "test_a_broken_audit_chain_is_reported_as_broken"),
    Mutation("the posture section stops admitting it has no history",
             "evidence.py",
             '"Current state only. Security status is stored one row per server "',
             '"Security posture over the reporting period. "',
             "test_posture_says_it_has_no_history"),
    Mutation("an empty firewall section stops saying why it is empty",
             "evidence.py",
             "None if ever else",
             "None if True else",
             "test_an_empty_firewall_section_says_which_kind_of_empty"),
    Mutation("the most common failure reason goes back to unrecognised",
             "evidence.py",
             '"0xc000006e": "account restriction (logon hours, workstation or expiry)",',
             '',
             "test_the_common_failure_reasons_are_named_not_left_as_hex"),
    Mutation("the spreadsheet prints a bare zero for an unread field",
             "reports_evidence.py",
             'return "not read" if v is None else ("yes" if v else "no")',
             'return "no" if v is None else ("yes" if v else "no")',
             "test_the_spreadsheet_says_not_read_rather_than_zero"),
    Mutation("a PDF cell stops escaping stored text",
             "reports_evidence.py",
             'return Paragraph(escape(str(text if text is not None else ""))[:400], style)',
             'return Paragraph(str(text if text is not None else "")[:400], style)',
             "test_stored_text_is_escaped_before_it_reaches_a_paragraph"),
    Mutation("the A4 frame is widened so an oversized table fits",
             "reports_evidence.py",
             "FRAME_WIDTH = 481",
             "FRAME_WIDTH = 900",
             "test_no_evidence_table_is_wider_than_the_frame"),
)
...
SUITE_FILES = {
    ...
    "evidence-report": "tests/test_evidence_report.py",
